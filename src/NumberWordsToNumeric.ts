...
interface NumberWordsToNumericOptions {
 ...
 */
export function NumberWordsToNumeric(
  sentence: string,
  options?: NumberWordsToNumericOptions
): string {
  ...
  for (let l = 0; l < letters.length; l++) {
    ...
    if (/[ a-zA-Z]/.test(letter) && l !== letters.length - 1) {
      ...
      continue;
    } else if (/[ a-zA-Z]/.test(letter) && l === letters.length - 1) {
      // last
      wordsWithoutPunctuations += letter;
    }
    if (wordsWithoutPunctuations.length !== 0) {
      res =
        res +
        TranslateSentenceContainingOnlyWords(wordsWithoutPunctuations, options);
...
function TranslateSentenceContainingOnlyWords(
  sentence: string,
  options?: NumberWordsToNumericOptions
): string {
  if (typeof sentence !== "string") {
  ...
  for (let i = 0; i < innerStrings.length; i++) {
    ...
    if (!isStringNum && isNumberWordProcessRunning) {
      ...
      if (options?.numberFormat) {
        textToAdd = textToAdd.toLocaleString(
